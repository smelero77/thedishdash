...
const ModifierModalComponent = forwardRef<HTMLDivElement, ModifierModalProps>(
  (
    { isOpen, itemName, itemDescription, itemAllergens, modifiers, menuItems, onClose, onConfirm },
    ref,
  ) => {
    ...
    const scrollToModifier = useCallback(
      (modifierId: string) => {
    ...
    const showConfirmButton =
      modifiers.length > 1 || modifiers.some((m) => m.multi_select && m.options.length > 0);
            ...
            {modifiers.map((modifier) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <h3 className="text-lg font-semibold text-[#0e1b19] dark:text-white">
                      {modifier.name}
                    </h3>
                    {modifier.required && (
