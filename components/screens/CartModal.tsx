...
const CartModalComponent = forwardRef<HTMLDivElement, CartModalProps>(
  ({ onClose, currentClientAlias }, ref) => {
    ...
    const totalQuantity = actions?.getTotalItems() ?? 0;

    const handleQuantityChange = useCallback(
      (item: CartItem, increment: boolean) => {
        if (!actions) return;
                        ...
                        >
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-semibold text-[#0e1b19] text-base">
                                {item.item.name}
                              </p>
