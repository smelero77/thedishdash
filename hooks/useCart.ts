...
export async function addOrIncrementCartItem(
  ...
  console.log('[addOrIncrementCartItem] Calling increment_item', { temporaryOrderId, menuItemId, alias, modifiers });
...
export async function decrementOrDeleteCartItem(
  ...
  console.log('[decrementOrDeleteCartItem] Calling decrement_item', { temporaryOrderId, menuItemId, alias, modifiers });
...
function useCart(
    ...
    console.log('[useCart] Hook ejecutándose. Dependencias:', { hasMenuItems: !!menuItems, currentClientAlias, tableNumber });
    ...
    const calculateItemPrice = useCallback((item: MenuItemData, modifiers: SelectedModifiers | null): number => {
        ...
        const modifiersPrice = Object.values(modifiers).reduce((total: number, modifierGroup: SelectedModifier) => {
            if (modifierGroup && modifierGroup.options && Array.isArray(modifierGroup.options)) {
                const groupOptionPrice = modifierGroup.options.reduce(
                    (sum: number, option: SelectedModifierOption) => sum + (option.extra_price || 0), 0
                );
    ...
    const updateCartTotal = useCallback((currentCart: Cart, menuItemsData: MenuItemData[] | null) => {
        if (!menuItemsData) {
            console.warn("[updateCartTotal] menuItemsData no disponible, no se puede calcular el total.");
            setCartTotal(0);
            return;
        }
        ...
        const newTotal = Object.values(currentCart).reduce((sum, cartItem) => {
            ...
            if (!menuItem) {
                console.warn(`[updateCartTotal] MenuItemData not found for ID: ${cartItem.id}. Skipping price calculation.`);
                return sum;
            }
    ...
    // --- Efectos ---
    useEffect(() => {
        console.log('[useCart: InitEffect] triggered. Deps:', { tableNumber, hasMenuItems: !!menuItems });
        ...
        if (!menuItems || menuItems.length === 0) {
            console.log('[useCart: InitEffect] menuItems not available, skipping cart items processing, but will try init OrderID');
        }
                ...
                if (menuItems && menuItems.length > 0) {
                    ...
                    items?.forEach((item) => {
                        ...
                        if (menuItem) {
                            ...
                            console.warn(`[useCart: InitEffect] MenuItemData (ID: ${item.menu_item_id}) no encontrado en menuItems prop. Item no se añadirá.`);
                        }
                    });
                    ...
                    console.log('[useCart: InitEffect] menuItems no disponibles al procesar items, carrito inicializado vacío.');
                    setCart({});
                    setCartTotal(0);
                }
            })
    ...
    useEffect(() => {
        console.log('[useCart: RealtimeEffect] triggered. Deps:', { temporaryOrderId, hasMenuItems: !!menuItems });
        if (!temporaryOrderId || !menuItems || menuItems.length === 0) {
            console.log('[useCart: RealtimeEffect] No temporaryOrderId o menuItems no disponible, no se inicia la suscripción.');
            return;
        }
        ...
        const handleRealtimeChanges = (
            payload: RealtimePostgresChangesPayload<TemporaryOrderItem>
        ) => {
            ...
            const item = (payload.eventType === 'DELETE' ? payload.old : payload.new) as TemporaryOrderItem | undefined;

            if (!item || !item.temporary_order_id) {
                console.warn('[useCart: RealtimeEffect] Payload recibido sin datos válidos (new/old) o sin temporary_order_id.');
                return;
            }
        ...
        const channel = supabase
            ...
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'temporary_order_items', filter: `temporary_order_id=eq.${temporaryOrderId}` },
    ...
    const handleAddToCart = useCallback(async (itemId: string, modifiers: SelectedModifiers | null = null) => {
        if (!temporaryOrderId) {
