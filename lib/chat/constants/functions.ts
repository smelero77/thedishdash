        ...
        items: {
          ...
          properties: {
            id: {
              ...
              format: 'uuid',
              description:
                'El ID EXACTO de un item de la lista de candidatos proporcionada. CRÍTICO: Usa SOLO IDs que aparezcan en la lista de candidatos, NUNCA inventes IDs ni uses otros valores.',
            },
            reason: {
              type: 'string',
              description:
                'La razón por la que este item específico es una buena recomendación para el usuario. IMPORTANTE: Esta razón debe basarse ÚNICAMENTE en los atributos y características del plato con este ID específico (nombre, descripción, categorías, etc.). NO mezcles información con otros platos.',
            },
          },
          required: ['id', 'reason'],
        },
    ...
    parameters: {
      ...
      properties: {
        ...
        price_max: {
          type: 'number',
          description:
            "Precio máximo deseado por el usuario. Extraer de frases como 'menos de X', 'hasta X', 'no más de X'. Si se da un rango 'entre A y B', este es B.",
        },
        price_min: {
          type: 'number',
          description:
            "Precio mínimo deseado por el usuario. Extraer de frases como 'más de X', 'desde X', 'al menos X'. Si se da un rango 'entre A y B', este es A.",
        },
        keywords_include: {
          type: 'array',
          items: { type: 'string' },
