...
export const ExtractedFiltersSchema = z.object({
  item_type: z.enum(["Comida", "Bebida"]).optional().describe("Tipo de ítem general (Comida o Bebida)."),
  category_names: z.array(z.string().trim().min(1)).optional().describe("Nombres de categorías mencionadas (ej. 'Postres')."),
  exclude_allergen_names: z.array(z.string().trim().min(1)).optional().describe("Nombres de alérgenos a EVITAR (ej. 'Gluten')."),
  include_diet_tag_names: z.array(z.string().trim().min(1)).optional().describe("Nombres de etiquetas dietéticas que QUIERE (ej. 'Vegano')."),
  is_vegetarian_base: z.boolean().optional().describe("Usuario busca algo explícitamente vegetariano."),
  is_vegan_base: z.boolean().optional().describe("Usuario busca algo explícitamente vegano."),
  is_gluten_free_base: z.boolean().optional().describe("Usuario busca algo explícitamente sin gluten."),
  is_alcoholic: z.boolean().optional().describe("Para bebidas, preferencia por alcohólicas/no alcohólicas."),
  ...
  keywords_include: z.array(z.string().trim().min(1)).optional().describe("Otras palabras clave relevantes (ej. 'casero', 'para compartir')."),
