...
export async function buildPromptContext() {
  ...
  const categoriaSlot = categories.map(cat => ({
    name: cat.name,
    slots: slots.filter(slot => slot.categories?.some(c => c.name === cat.name)).map(s => s.name)
  }));
...
export function buildSystemPrompt(context: Awaited<ReturnType<typeof buildPromptContext>>) {
  ...
  const slotsPrompt = context.slots.map(s => `- ${s.name} (${s.start_time} - ${s.end_time})`).join('\n');
  const alergenosPrompt = context.alergenos.map(a => `- ${a}`).join('\n');
  const dietasPrompt = context.dietas.map(d => `- ${d}`).join('\n');
  const slotCategoriaPrompt = context.slotCategoria.map(
    s => `- ${s.name}: ${s.categorias.join(', ')}`
  ).join('\n');
